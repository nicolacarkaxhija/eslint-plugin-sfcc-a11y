...
const { CONTENT_SENTINEL } = require('./isml.js');
 ...
 * ISML content sentinels injected by the sanitizer for <isprint> tags.
 * @param {{ children?: any[] }} node
 * @returns {boolean}
 */
function hasVisibleTextContent(node) {
  const text = getTextContent(node);
  return text.includes(CONTENT_SENTINEL) || text.trim().length > 0;
}
