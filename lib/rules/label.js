  ...
  create(context) {
    ...
    return {
      'Tag:exit'(node) {
        ...
        // Collect <label for="..."> entries
        if (name === 'label') {
          const forAttr = getAttr(node, 'for');
          if (forAttr && forAttr !== true && !isDynamic(forAttr)) {
            labelState.forLabels.set(forAttr, true);
          }
          // A label that contains a form control directly — mark it
      ...
      'Program:exit'() {
        for (const node of controls) {
          ...
          // aria-label or aria-labelledby — valid
          const ariaLabelAttr = node.attributes?.find(
            (a) => a.key?.value?.toLowerCase() === 'aria-label',
          );
          if (ariaLabelAttr?.value && isDynamic(ariaLabelAttr.value.value)) continue;
          if (hasAriaLabel(node)) continue;

          // Dynamic id — cannot validate for/id association; skip
          const id = getAttr(node, 'id');
          if (id && isDynamic(id)) continue;

          // for/id association in the same file
          if (id && labelState.forLabels.has(id)) continue;

          // Wrapped by a <label> element — check if any wrapping label contains this node
