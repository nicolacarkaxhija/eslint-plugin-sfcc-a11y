...
describe('hasVisibleTextContent', () => {
  ...
  it('returns true when text contains the ISML content sentinel', () => {
    expect(hasVisibleTextContent(tag('h1', {}, [text('__ISML_CONTENT__')]))).toBe(true);
  });
});
...
describe('hasAriaLabel', () => {
  ...
  it('returns true when aria-label is a boolean attribute (true)', () => {
    // Boolean attribute — value is `true` (not a string), treated as no-value
